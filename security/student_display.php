...
// View-only page showing camera feed and digital ID from the security guard's gate monitor.
// Opens in a new tab/window, typically on a second screen facing students at the gate.
// Data is received via BroadcastChannel (same-origin) from the guard's gate_monitor.php.
// No authentication required — this page has no backend data access.
...
header("Content-Security-Policy: default-src 'self'; script-src 'self' 'unsafe-inline' https://cdn.tailwindcss.com https://cdn.jsdelivr.net; style-src 'self' 'unsafe-inline' https://cdn.tailwindcss.com; img-src 'self' data: https:; font-src 'self' data:; connect-src 'self'; frame-ancestors 'none';");
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
let clearTimer      = null;
let popupFaceSystem = null;
let popupFaceReady  = false;
let popupInitPromise = null;
let popupScanRequested = false;
let popupCsrfToken = '';
let popupFaceConfig = null;
let popupTakenOver = false;
...
function stopPopupRecognition() {
    if (popupFaceSystem && popupFaceSystem._continuousRunning) {
        popupFaceSystem.stopContinuousDetection();
    }
    // Clear popup overlay so broadcast frames show through cleanly
    if (feedOverlay) {
        var ovCtx = feedOverlay.getContext('2d');
        ovCtx.clearRect(0, 0, feedOverlay.width, feedOverlay.height);
    }
    if (popupTakenOver) {
        popupTakenOver = false;
        channel.postMessage({ type: 'popup_release' });
    }
}

// Begin preloading as soon as config is available (don't wait for focus).
function startPopupPreload() {
    if (popupInitPromise) return;
    if (!popupCsrfToken || !popupFaceConfig) return;
    if (typeof FaceRecognitionSystem === 'undefined' || typeof faceapi === 'undefined') {
        // Scripts still loading — retry shortly
        setTimeout(startPopupPreload, 500);
        return;
    }
    popupInitPromise = (async function () {
        try {
            const system = new FaceRecognitionSystem({
                modelPath: popupFaceConfig.modelPath,
                matchThreshold: popupFaceConfig.matchThreshold,
                minConfidence: popupFaceConfig.minConfidence,
                csrfToken: popupCsrfToken,
                detectionIntervalMs: popupFaceConfig.detectionIntervalMs,
                ssdInputSize: popupFaceConfig.ssdInputSize,
                livenessEnabled: false, // Disabled: broadcast frames from background tab are static, liveness sees no movement
                requiredConsecutiveFrames: popupFaceConfig.requiredConsecutiveFrames,
                livenessMinFrames: popupFaceConfig.livenessMinFrames,
                unrecognizedFramesThreshold: popupFaceConfig.unrecognizedFramesThreshold,
                livenessFailFramesThreshold: popupFaceConfig.livenessFailFramesThreshold,
                minFaceSizePx: popupFaceConfig.minFaceSizePx,
                minFaceSizeRatio: popupFaceConfig.minFaceSizeRatio,
                descriptorChunkSize: popupFaceConfig.descriptorChunkSize,
                tickWorkerPath: popupFaceConfig.tickWorkerPath,
                onDetection: function(result) {
                    // Forward result to security page.
                    channel.postMessage({ type: 'student_detection_result', result: result });
                    // Only stop popup detection on a successful match.
                    // not_recognized / liveness_failed should keep scanning (mirrors security tab).
                    if (result.matched) {
                        stopPopupRecognition();
                    }
                },
                onError: function() {},
            ...
            const modelsOk = await system.loadModels();
            if (!modelsOk) return false;

            const count = await system.loadKnownFaces('../api/get_face_descriptors.php');
            if (count < 0) return false;

            system.videoElement = feedCanvas;
            system.canvasElement = feedOverlay;
            popupFaceSystem = system;
            popupFaceReady = true;
            return true;
        } catch (e) {
            return false;
...
async function maybeStartPopupRecognition() {
    if (!popupScanRequested) return;
    if (document.visibilityState !== 'visible' || !document.hasFocus()) return;
    if (!feedCanvas.width || !feedCanvas.height) return;
    if (!popupFaceReady) {
        // Engine not ready yet — preload was started, just wait
        if (popupInitPromise) await popupInitPromise;
        if (!popupFaceReady) return;
    }
    if (popupFaceSystem._continuousRunning) return;
    // Tell security to pause its detection — we're taking over
    if (!popupTakenOver) {
        popupTakenOver = true;
        channel.postMessage({ type: 'popup_takeover' });
    }
    popupFaceSystem.videoElement = feedCanvas;
    popupFaceSystem.startContinuousDetection();
}
...
channel.onmessage = function (e) {
    ...
    switch (msg.type) {
        case 'connected':
            popupCsrfToken = msg.csrfToken || popupCsrfToken;
            popupFaceConfig = msg.faceConfig || popupFaceConfig;
            popupScanRequested = !!msg.detectionRunning;
            ...
            // Eagerly preload face engine in background so it's ready instantly on focus
            startPopupPreload();
            maybeStartPopupRecognition();
            break;
    ...
    resultPanel.innerHTML = `
        <div class="p-5 fade-in">
            ...
            <div class="mt-3 bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-3 text-center">
                <p class="text-yellow-300 text-sm font-medium">Violation #${escHtml(String(s.violation_count))}</p>
                <p class="text-yellow-200/70 text-xs">${escHtml(s.severity_message || '')}</p>
            </div>` : ''}
        </div>`;
    ...
    resultPanel.innerHTML = `
        <div class="p-5 fade-in">
            ...
            <div class="bg-red-500/10 border border-red-500/30 rounded-xl p-4 text-center">
                ...
                <p class="text-red-200/60 text-xs mt-2">Please contact the Administration Office.</p>
            </div>
        </div>`;
